	...
	public function postProcess()
	{
		...
		switch ($task) {
			case "redirect":
				...
				$order_id = $jokulva->get_order_id($_POST['ORDERID']);

				$extra_vars = array(
                    '{order_name}' => $_POST['TRANSIDMERCHANT']
                );
				if (!$order_id) {
					$order_state 				  = $config['DOKU_INITIALIZE_PAYMENT'];
					$trx['amount']                = $_POST['AMOUNT'];
					$jokulva->validateOrder($_POST['ORDERID'], $order_state, $trx['amount'], $jokulva->displayName,'', $extra_vars);
					$order_id = $jokulva->get_order_id($_POST['ORDERID']);
				}
				...
				Configuration::updateValue('PAYMENT_EXP', $newDate->format('d M Y H:i'));
